...
def list_reviews(
    ...
    return PagedResponse.from_query(db.query(Review), limit=limit, offset=offset)
...
def list_issues(
    ...
    """Issue 목록. 캐시 적재 성공 시 캐시 우선, 실패 시 DB fallback."""
    if cache.loaded:
    ...
    return PagedResponse.from_query(db.query(Issue), limit=limit, offset=offset)
...
def list_gate_rules(
    ...
    return PagedResponse.from_query(db.query(GateRule), limit=limit, offset=offset)
