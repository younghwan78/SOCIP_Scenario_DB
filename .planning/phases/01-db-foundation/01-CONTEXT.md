...
- **D-07:** ORM → Pydantic 매핑: `model_validate(orm_obj)` 패턴 + `ConfigDict(from_attributes=True)`. ORM 객체를 직접 전달. `row.__dict__` 사용 금지 — `_sa_instance_state` 키가 포함되어 `extra='forbid'` 설정에서 ValidationError 발생. 불일치 필드(alias 컬럼)는 Python 속성명으로 Pydantic 필드명 정의.
