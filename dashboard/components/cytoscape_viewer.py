...
def _build_stylesheet(canvas_h: int) -> list[dict]:
    ...
    styles.append({
        ...
        "style": {"width": 3.5, "overlay-color": "#1D4ED8", "overlay-opacity": 0.10},
    })
...
def _build_html(
    ...
    layout_nodes = _build_layout_nodes()
...
(function() {{
  ...
  cy.ready(function() {{
    // Force canvas resize (iframe may report wrong size initially)
    cy.resize();

    // Fit to functional nodes only (exclude layout elements)
    ...
    // Show debug bar briefly then hide (keeps visible if user inspects)
    dbg.textContent = info;
    dbg.style.display = 'block';
    setTimeout(function() {{ dbg.style.display = 'none'; }}, 4000);
  }});
